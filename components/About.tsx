  ...
  return (
    <section id="about" className="py-20 bg-white" aria-labelledby="about-heading">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 id="about-heading" className="mb-4 text-[#1C3666] text-3xl sm:text-4xl lg:text-[2.7rem] font-semibold">
            Chi Sono
          </h2>
          <p className="text-[#304E8E] text-sm sm:text-base">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8 max-w-6xl mx-auto">
          {skills.map((skill, index) => (
            <motion.div
              ...
              className="group relative p-4 sm:p-6 bg-[#BBDBE6]/30 border border-[#80B2C8] hover:border-[#304E8E] hover:bg-white transition-all duration-300"
            >
              {/* Decorative corner */}
              ...
              <skill.icon className="w-8 sm:w-10 h-8 sm:h-10 text-[#304E8E] mb-3 sm:mb-4" />
              <h3 className="mb-2 text-[#1C3666] text-sm sm:text-base">
                {skill.title}
              </h3>
              <p className="text-[#4072AB] text-sm">
                {skill.description}
              </p>
            </motion.div>
          ))}
